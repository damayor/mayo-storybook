...
type ShapeDataLocal = {
  vertices: number[]; colors: number[]; pickColors: number[];
  indices: number[]; indexCount: number; primitiveType: GLenum;
};
...
export const WebGLCanvas: React.FC<WebGLCanvasProps> = ({
  ...
  const floorData  = useRef<{ vertices: number[]; colors: number[]; indices: number[] } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!shader) { console.error('Shader init failed'); return; }
    ...
    floorBuf.current = initLinesBuffers(gl, {
      vertices: fd.vertices, colors: fd.colors, pickColors: fd.colors,
      indices: fd.indices, indexCount: fd.indices.length, primitiveType: gl.LINES,
    });

    // Cube edges (positions updated each frame)
    const verts = stateRef.current.cubeVertices;
    const edgeColors = Array<number>(8 * 4).fill(0).map((_, i) => [0.2,0.5,1,1][i % 4]);
    edgeData.current = {
      vertices: [...verts], colors: edgeColors, pickColors: edgeColors,
      indices: [0,1, 1,2, 2,3, 3,0, 4,5, 5,6, 6,7, 7,4, 0,4, 1,5, 2,6, 3,7],
      indexCount: 24, primitiveType: gl.LINES,
    };
    ...
    pointData.current = {
      ...
      indices: [0,1,2,3,4,5,6,7], indexCount: 8, primitiveType: gl.POINTS,
    };
    ...
    faceData.current = {
      vertices: fInit.vertices, colors: fInit.colors, pickColors: fInit.colors,
      indices: fInit.indices, indexCount: fInit.indices.length, primitiveType: gl.TRIANGLES,
    };
    ...
    triData.current = {
      vertices: tInit.vertices, colors: tInit.colors, pickColors: tInit.colors,
      indices: tInit.indices, indexCount: tInit.indices.length, primitiveType: gl.TRIANGLES,
    };
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (pointData.current && pointBuf.current) {
        const pc = getCubePointColors(s.hoveredVertexIndex, s.selectedVertexIndex, s.isDraggingVertex);
      ...
      if (s.viewMode !== 'points' && floorBuf.current && floorData.current) {
        ...
        drawShape(gl, shader, floorBuf.current, {
          vertices: fd.vertices, colors: fd.colors, pickColors: fd.colors,
          indices: fd.indices, indexCount: fd.indices.length, primitiveType: gl.LINES,
        }, true);
      }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isDragVert.current && s.selectedVertexIndex !== null) {
      ...
      if (world) {
        ...
        onLog(`Vertex ${s.selectedVertexIndex}: (${world.x.toFixed(2)}, ${world.y.toFixed(2)}, ${targetZ.toFixed(2)})`);
      }
      return;
    }
    ...
    const mvp = getMVP();
    if (!mvp) return;
    const hit = pickVertex(pos.x, pos.y, canvas.width, canvas.height, s.cubeVertices, mvp, PICK.RADIUS_PX);
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const mvp = getMVP();
    if (!mvp) return;
    const hit = pickVertex(pos.x, pos.y, canvas.width, canvas.height, stateRef.current.cubeVertices, mvp, PICK.RADIUS_PX);
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    if (e.button === 2) { isRightDrag.current = false; return; }
  ...
  const handleMouseLeave = useCallback(() => {
    ...
    if (isDragVert.current) { isDragVert.current = false; onStopDrag(); }
    onHoverVertex(null);

...
export function initLinesBuffers(gl: WebGLRenderingContext, data: ShapeData): BufferInfo {
  ...
  return { position: positionBuffer, color: colorBuffer, pickColor: pickColorBuffer, indices: indexBuffer };
}

export function updateBuffer(gl: WebGLRenderingContext, buffer: WebGLBuffer | null, data: Float32Array): void {
  if (!buffer) return;
...
export function setupDrawing(gl: WebGLRenderingContext, shaderInfo: ShaderInfo, projection: Mat4, modelView: Mat4): void {
...
function divideByW(v: Float32Array): void {
  if (v[3] !== 0 && v[3] !== 1) { v[0]/=v[3]; v[1]/=v[3]; v[2]/=v[3]; v[3]=1; }
}

// ── Picking ───────────────────────────────────────────────────────────────────

export interface PickResult { vertexIndex: number; distance: number; }
...
export function getCubePointColors(
  ...
  for (let i = 0; i < count; i++) {
    const c =
      isDragging && i === selectedIndex ? VERTEX_COLORS.DRAGGING :
      i === selectedIndex               ? VERTEX_COLORS.SELECTED :
      i === hoveredIndex                ? VERTEX_COLORS.HOVER    :
...
export function generateFloorGrid(dim = FLOOR.DIM, lines = FLOOR.LINES): {
  vertices: number[]; colors: number[]; indices: number[];
} {

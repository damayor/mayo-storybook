...
import { orbitMaxPolarAngle, orbitMinPolarAngle } from '../../non-stories-components/controls/controls.config'
...
export const getShortestWayAngles = (startBeta: number, endBeta: number) => {
  ...
  const fixedStartBeta = anglesDifference > Math.PI && startBeta > Math.PI ? startBeta - 2 * Math.PI : startBeta
  const fixedEndBeta = anglesDifference > Math.PI && endBeta > Math.PI ? endBeta - 2 * Math.PI : endBeta
...
export const getModalAnchorOrigin = (el: Object3D, camera: Camera, size: { width: number; height: number }) => {

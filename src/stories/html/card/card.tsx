    ...
    >
      <div
        className={`relative w-full h-full transition-transform duration-700 transform-style-3d 
          md:transform-style-3d ${isFlipped ? 'md:rotate-y-180' : ''}`}
        style={{
      ...
      >
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="h-48 w-full overflow-hidden bg-gray-100">
              <img src={picture} alt={projectTitle} className="w-full h-full object-cover" />
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <p className="text-gray-600 text-sm leading-relaxed line-clamp-4">{subtitle}</p>

            <div className="flex flex-wrap gap-2 mb-4 mt-2 justify-center">
              {tags.map((tool, i) => (
                <span
                  key={i}
                  className="px-3 py-1 bg-camelot-700/20 text-camelot-800 rounded-full text-sm"
                >
                  {tool}
                </span>
              ))}
            ...
            <div className="md:hidden flex flex-wrap py-4 gap-4 justify-center">
              {technologies.map((tech) => (
                <Icon size="small" technology={tech} />
              ))}
            </div>
            {ctaLink && (
        ...
        >
          <div className="flex flex-wrap gap-6 mb-8 justify-center">
            {technologies.map((tech) => (
              <Icon size="medium" technology={tech} className="mb-2" />
            ))}
          </div>
